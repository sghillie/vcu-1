...
static void
USBD_FrameWork_AssignEp(USBD_DevClassHandleTypeDef *pdev, uint8_t Add, uint8_t Type, uint32_t Sze);

#if USBD_MSC_CLASS_ACTIVATED == 1U
static void
USBD_FrameWork_MSCDesc(USBD_DevClassHandleTypeDef *pdev, uint32_t pConf, uint32_t *Sze);
    ...
    while (Idx_Instance < USBD_MAX_SUPPORTED_CLASS)
    {
            ...
            (pdev->NumClasses < USBD_MAX_SUPPORTED_CLASS))
        {
            ...
            (void)USBD_FrameWork_AddClass(
                pdev, (USBD_CompositeClassTypeDef)UserClassInstance[Idx_Instance],
...
uint8_t USBD_FrameWork_AddToConfDesc(USBD_DevClassHandleTypeDef *pdev, uint8_t Speed, uint8_t *pCmpstConfDesc)
{
    ...
    case CLASS_TYPE_MSC:
        ...
        {
            /* Assign IN Endpoint */
            USBD_FrameWork_AssignEp(pdev, USBD_MSC_EPIN_ADDR, USBD_EP_TYPE_BULK,
...
static void USBD_FrameWork_MSCDesc(USBD_DevClassHandleTypeDef *pdev, uint32_t pConf, uint32_t *Sze)
{
    ...
    __USBD_FRAMEWORK_SET_EP((pdev->tclasslist[pdev->classId].Eps[0].add), (USBD_EP_TYPE_BULK),
                            (uint16_t)(pdev->tclasslist[pdev->classId].Eps[0].size),
                            (0U), (0U));

    /* Append Endpoint descriptor to Configuration descriptor */
    __USBD_FRAMEWORK_SET_EP((pdev->tclasslist[pdev->classId].Eps[1].add), (USBD_EP_TYPE_BULK),
