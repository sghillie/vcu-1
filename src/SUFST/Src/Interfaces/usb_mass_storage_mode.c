...
static UINT msc_media_read(VOID *storage, ULONG lun, UCHAR *data_pointer,
                           ULONG number_blocks, ULONG lba, ULONG *media_status)
{
    (void)storage; (void)lun; (void)media_status;
    if ((uint32_t)data_pointer % 4 != 0)
    {
        LOG_ERROR("msc_read: buf %p unaligned\n", data_pointer);
        return UX_ERROR;
    }
    HAL_StatusTypeDef dma_status = HAL_SD_ReadBlocks_DMA(&hsd1, data_pointer,
                                                          (uint32_t)lba,
                                                          (uint32_t)number_blocks);
    if (dma_status != HAL_OK)
    {
        LOG_ERROR("msc_read: DMA failed lba=%lu nblk=%lu halstate=%d err=0x%lx\n",
                  lba, number_blocks, (int)HAL_SD_GetState(&hsd1), HAL_SD_GetError(&hsd1));
        return UX_ERROR;
    }
...
static UINT msc_media_write(VOID *storage, ULONG lun, UCHAR *data_pointer,
                            ULONG number_blocks, ULONG lba, ULONG *media_status)
{
    (void)storage; (void)lun; (void)media_status;
    if (HAL_SD_WriteBlocks_DMA(&hsd1, data_pointer, (uint32_t)lba, (uint32_t)number_blocks) != HAL_OK)
        return UX_ERROR;
                      ...
                      const config_usb_msc_t *config_ptr)
{
    ctx->pool = pool;
    void *stack_ptr = NULL;
    if (tx_byte_allocate(pool, &stack_ptr,
                         config_ptr->thread.stack_size, TX_NO_WAIT) != TX_SUCCESS)
        return STATUS_ERROR;
    if (tx_thread_create(&ctx->thread, (CHAR *)config_ptr->thread.name,
                         usb_msc_thread_entry, (ULONG)ctx,
...
status_t enter_usb_mass_storage_mode(TX_BYTE_POOL *pool)
{
    ...
    if (ux_device_stack_initialize(UX_NULL, 0,
    ...
    UX_SLAVE_CLASS_STORAGE_LUN *lun = &storage_param.ux_slave_class_storage_parameter_lun[0];
    ...
    storage_param.ux_slave_class_storage_parameter_vendor_id      = (UCHAR *)"SUFST   ";
    storage_param.ux_slave_class_storage_parameter_product_id     = (UCHAR *)"VCU SD Card     ";
    storage_param.ux_slave_class_storage_parameter_product_rev    = (UCHAR *)"1.0 ";
    storage_param.ux_slave_class_storage_parameter_product_serial = (UCHAR *)"00000001";
